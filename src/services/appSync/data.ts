...
export interface RawAwsAppSync extends AppSync.GraphqlApi {
...
const listGraphqlApiData = async (
  ...
  try {
    ...
    while (nextToken) {
      graphqlApis = await appSync
        .listGraphqlApis({ nextToken })
        .promise()
      fullResources.push(...graphqlApis.graphqlApis)
...
const listApiKeysData = async (
  ...
  try {
    ...
    while (nextToken) {
      apiKeys = await appSync
        .listApiKeys({ apiId, nextToken })
        .promise()
      fullResources.push(...apiKeys.apiKeys)
...
const listFunctionsData = async (
  ...
  try {
    ...
    while (nextToken) {
      functions = await appSync
        .listFunctions({ apiId, nextToken })
        .promise()
      fullResources.push(...functions.functions)
...
const listResolversByFunction = async (
  ...
  try {
    ...
    let resolvers = await appSync.listResolversByFunction({ apiId, functionId }).promise()
...
export default async ({
  ...
  for (const region of regions.split(',')) {
    ...
    for (const graphqlApi of graphqlApis) {
      ...
      const dataSourceList = await listDataSourcesData(appSync, graphqlApi.apiId)
      ...
      for (const functionData of functionList) {
        const resolvers = await listResolversByFunction(appSync, graphqlApi.apiId, functionData.functionId)
        functions.push({
          ...functionData,
          resolvers,
        })
      ...
      for (const typeData of typeList ) {
        const resolvers = await listTypesResolverData(appSync, graphqlApi.apiId, typeData.name)
        types.push({
          ...typeData,
          resolvers,
        })
